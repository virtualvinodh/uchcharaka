...
def text_compare_reversible_2(source_script, target_script, source_text):
    try:
        with open('test_texts/source_Devanagari'+ '_' + source_text + '.txt', 'r', encoding= 'utf8') as fi:
            src_txt = fi.read()
            dev_to_src = transliterate.process('Devanagari', source_script, src_txt, =False)
            src_to_tgt = transliterate.process(source_script, target_script, dev_to_src, =False)
            tgt_to_src = transliterate.process(target_script, source_script, src_to_tgt, =False)
...
def text_compare_reversible(source_script, target_script, source_text):
    try:
        with open('test_texts/source_' + source_script + '_' + source_text + '.txt', 'r', encoding= 'utf8') as fi:
            src_txt = fi.read()
            src_to_tgt = transliterate.process(source_script, target_script, src_txt, =False)
            tgt_to_src = transliterate.process(target_script, source_script, src_to_tgt, =False)
...
class TestSanity:
    ...
    def test_sanity_default(self, target_scripts):
        ...
        try:
            ...
            for testcase in testcases:
                for case in testcase["cases"]:
                     = testcase[""] if "" in testcase else True
                    assert transliterate.process(testcase["source"], testcase["target"], case[0], =) == case[1]

                    if "reverse" in testcase:
                        assert transliterate.process(testcase["target"], testcase["source"], case[1], =) == case[0]
        except KeyError:
            assert 1 == 1
...
class TestMisc:
    ...
    def test_bengali_va_preserve_source(self):
        assert transliterate.process('HK', 'Bengali', 'savva sabba samba samva sarva sarba ukva ukba utva utba udveda udbodhana bru bra bya byu vra vya vla', =False) == 'সভ়্ভ় সব্ব সম্ব সম্ভ় সর্ভ় সর্ব উক্ব উক্‌ব উত্ব উৎ‌ব উদ্বেদ উদ্‌বোধন ব্রু ব্র ব্য ব্যু ভ়্র ভ়্য ভ়্ল'
